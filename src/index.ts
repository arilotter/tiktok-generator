...
import { getImage, getPalette } from "./imageMaker";
...
async function main() {
	...
	for (let i = 0; i < postBody.length; i++) {
		...
		const audio = getSpeech(expandTextForSpeaking(text), speaker).then(
			(speech) => Bun.write(audioFile, speech),
		);
		const image = getImage(
			...
			colors.bgColor,
			colors.bgBorder,
		).then((image) => Bun.write(imageFile, image));
	...
	const start = Math.round(
		startRange < 1 ? 0 : Math.random() * (startRange - 1),
	);
